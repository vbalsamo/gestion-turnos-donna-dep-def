    ...
    <div class="row align-items-stretch">
        ...
        <div class="col-md-7 col-xl-6 rounded-end">
            ...
            <form action="{{ route('register.store') }}" method="POST" class="mx-4">
                ...
                <span id='alerta'></span>
                <div class="mb-4">
                    <input disabled type="password" class="form-control" name="confirm_password" id="confirm_password" placeholder="Confirmar contraseña" required>
                </div>
...
/* todavia no funciona bien, es para comprobar si los campos 'contraseña' y 'confirmar contraseña' son iguales
    $('#password, #confirm_password').on('keyup', function () {
        if ($('#password').val() == $('#confirm_password').val()) {
            $('#alerta').html('Las contraseñas coinciden').css('color', 'green');
            $('#password, #confirm_password').attr('class','form-control is-valid');
        } else
            $('#alerta').html('Las contraseñas no coinciden').css('color', 'red');
